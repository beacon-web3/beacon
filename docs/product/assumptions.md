...
| Historical recommender stake additions | Original discoverer and prior reactivators may add locked SOL for future upvote/support credit share; they may also reactivate a deactivated recommendation and remain historical recommenders | Accepted | Later top-ups above an existing qualifying balance must be at least `0.05 SOL`; any recommender participant holding at least the `0.2 SOL` minimum keeps the cycle active. Extra stake is weighted linearly by locked SOL share per `docs/decisions/0024-linear-reward-weighting-across-recommender-stake.md` (superseding `docs/decisions/0016-diminishing-returns-for-extra-recommender-stake.md`); exact split parameters remain unresolved. See also `docs/decisions/0017-recommender-stake-balance-and-top-up-minimums.md`. |
| Recommendation inactivity window | No active recommender participant plus 90 days with no new support | Accepted | Recommender participants become inactive through reclaim or withdrawal. See `docs/decisions/0013-recommendation-inactivity-window.md`. |
